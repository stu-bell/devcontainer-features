...
- src/neovim can use git to clone neovim config into a directory. we do this by setting XDG_CONFIG_HOME to /config, however, this causes problems for other users.
- default config location should be user's home `~/.config/nvim`, but configurable by devcontainer feature option
- change install.sh to to clone into the remote user's home directory, either using remote user home env var, or util function remote_user_run when cloning
